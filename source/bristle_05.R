...
version <- 5         # increment to 2 for "my-art-system_02.R", etc
...
art_generator <- function(seed) {
  ...
  amp <- stats::runif(n_strokes, min = .3, max = .9)
  turns <- stats::runif(n_strokes, min = .5, max = 3)

  bg <- sketchpad::fill_solid(
    sketchpad::color_lighten(palette[length(palette)], amount = .4)
  )
