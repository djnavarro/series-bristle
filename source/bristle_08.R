...
version <- 8         # increment to 2 for "my-art-system_02.R", etc
...
art_generator <- function(seed) {
  ...
  locs_tny <- tidyr::expand_grid(
    x_mid = seq(-5.5, 5.5, by = .5),
    y_mid = seq(-2.5, 2.5, by = .5)
  ) |> 
    dplyr::mutate(
      x_min = x_mid - .25,
      y_min = y_mid - .25,
  ...
  n_sml <- nrow(locs_sml)
  n_lrg <- nrow(locs_lrg)
  n_all <- n_sml + n_lrg + n_tny

  locs_all <- dplyr::bind_rows(locs_lrg, locs_sml, locs_tny) |> 
  ...
  locs_valid <- locs_all |> 
    ...
    dplyr::slice_sample(prop = .5, replace = FALSE)
  ...
  amp <- sample(1:4, n_strokes, TRUE) * stroke_sz * 1.25
