...
version <- 16        # increment to 2 for "my-art-system_02.R", etc
...
art_generator <- function(seed) {
  ...
  bg_col <- sketchpad::color_lighten(palette[length(palette)], amount = runif(1, min = .3, max = .6))
  bg <- sketchpad::fill_solid(color = bg_col)
  palette2 <- palette |> 
    sketchpad::color_desaturate(amount = .2) |> 
    sketchpad::color_darken(amount = .75)
